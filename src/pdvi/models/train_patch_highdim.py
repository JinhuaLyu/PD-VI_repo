...
def estimate_sigma0_sigma1_from_initlabels(y: torch.Tensor, initial_labels: torch.Tensor, eps=1e-12):
    ...
    return sigma0_diag, sigma1_diag, mean_y, var_within
...
if __name__ == "__main__":
    ...
    # ------------- sigma estimates -------------
    sigma0_diag, sigma1_diag, initial_k_mean, initial_k_var = estimate_sigma0_sigma1_from_initlabels(y, initial_labels)
    ...
    m0 = initial_k_mean
    rho0 = torch.log(initial_k_var + 1e-12)
    ...
    for it in range(max_iter + 1):
        ...
        # ---------- sample ONE non-empty patch (simplest) ----------
        b = nonempty_patches[torch.randint(0, len(nonempty_patches), (1,), device=device).item()]
        idx = patch_indices[b]                 # indices of points in patch b
        n_b = idx.numel()
        ...
        # write back per-point alpha
        alpha_all[idx] = alpha_patch_new

        # ---------- primal residual (patch) ----------
        lambda_diff_b = lambda_b - lambda_0                      # (2*K*d,)
        primal_residual = torch.linalg.norm(lambda_diff_b)

        # ---------- dual update (patch) ----------
        mu_patch[b, : K * d] += lambda_diff_b[: K * d] / eta_m
        mu_patch[b, K * d :] += lambda_diff_b[K * d :] / eta_s

        # ---------- YOUR h_t and (m0,rho0) update, patch-consistent ----------
        # Patch-consistent aggregation: because L_b sums over i in S_b,
        # the "diff contribution" scales with |S_b|.
        #
        # Define:
        #   m_diff = |S_b| * (m_b - m0)
        #   rho_diff = |S_b| * (rho_b - rho0)
        # then your update:
        #   m_diff_mean = m_diff / n
        #   rho_diff_mean = rho_diff / n
        #   h_t_m += m_diff_mean
    ...
    plt.savefig(figures_dir / "mfvi_highdim_patch_labels.png")
